...
def lrt_stops
  ...
  FROM stops JOIN routes ON stops.id = routes.stop_id
...
def craiglockhart_to_sighthill
  ...
  stops AS stopa
  JOIN
  routes AS a ON (stopa.id = a.stop_id)
  JOIN
  routes AS b ON (a.company = b.company AND a.num = b.num)
  JOIN
  stops AS stopb ON (b.stop_id = stopb.id)
  ...
  JOIN
  routes AS d ON (d.company = c.company AND d.num = c.num)
  JOIN
  stops AS stopc ON (d.stop_id = stopc.id)
  WHERE
  stopa.name = 'Craiglockhart' AND stopb.name != 'Sighthill' AND stopc.name = 'Sighthill'
  SQL
end
